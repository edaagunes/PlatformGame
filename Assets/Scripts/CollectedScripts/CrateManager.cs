    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("kilicVurusBox"))
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    Vector3 randomVector = new Vector3(transform.position.x+(i-1),transform.position.y,transform.position.z);
                    ...
                    coin.GetComponent<Rigidbody2D>().velocity = patlamaMiktari * new Vector2(UnityEngine.Random.Range(1, 3),transform.localScale.y + UnityEngine.Random.Range(0,3));              
                }
            }
